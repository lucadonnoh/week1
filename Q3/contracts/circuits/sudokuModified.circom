// [assignment] please copy the entire modified sudoku.circom herpragma circom 2.0.3;
...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rp_puz[n] = RangeProof(4);
            rp_puz[n].range[0] <== 0;
            rp_puz[n].range[1] <== 9;
            rp_sol[n] = RangeProof(4);
            rp_sol[n].range[0] <== 0;
            rp_sol[n].range[1] <== 9;
